...
def login_page():
    st.title("🔒 AI量化最终完全体 · baostock+飞书版")
...
# ======================
# 数据工具（baostock 替代东方财富，A股专用，无IP限制）
# ======================
def get_data(code, start="2020-01-01", end="2026-12-31"):
    try:
        # baostock 登录（免费，无需Token）
        lg = bs.login()
        if lg.error_code != '0':
            print(f"baostock登录错误: {lg.error_msg}")
            return None
        
        # 获取A股日线数据（后复权）
        rs = bs.query_history_k_data_plus(
            f"{code}.SH" if len(code) == 6 else code,
            "date,close,volume",
            start_date=start,
            end_date=end,
            frequency="d",
        ...
        while (rs.next()):
            data_list.append({
                ...
                "close": float(rs.get_row_data()[1]),
                "volume": float(rs.get_row_data()[2])
            })
...
def get_sh_index():
    # 上证指数代码 000001.SH
    return get_data("000001.SH", start="2020-01-01")

# ======================
# 特征体系（空数据拦截，彻底解决 ValueError）
# ======================
...
if not st.session_state.login:
    ...
    st.sidebar.title("🧧 最终完全体 · baostock+飞书")
    ...
    if menu == "🏠 首页":
        ...
        st.title("📩 飞书推送测试")
        if st.button("立即发送今日报告到飞书"):
            report = generate_rich_report()
            if send_feishu("AI量化测试推送", report):
                st.success("✅ 推送成功！")
            else:
        ...
        **使用说明：**
        ...
        4. 数据接口：baostock（免费无Token，无IP限制）
        """)
