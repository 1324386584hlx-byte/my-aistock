...
# ======================
# 数据工具（akshare 替代 yfinance，A股专用）
# ======================
def get_data(code, start="20200101"):
    try:
        # akshare 直接获取 A 股日线数据，无需 .SS 后缀
        df = ak.stock_zh_a_daily(symbol=code, start_date=start, adjust="hfq")
...
def get_sh_index():
    # 上证指数代码 000001，用 akshare 获取
    return get_data("000001", start="20200101")
...
def super_signal(code):
    df = get_data(code, start="20250101")
