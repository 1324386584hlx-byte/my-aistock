...
def send_feishu(title, content):
    ...
    try:
        ...
        response = requests.post(FEISHU_WEBHOOK_URL, json=payload, headers=headers, timeout=8)
...
def get_data(code, start="20200101"):
    try:
        df = pro.daily(ts_code=code, start_date=start, adj='qfq')
        df.rename(columns={"close": "close", "vol": "volume", "trade_date": "date"}, inplace=True)
...
def get_sh_index():
    return get_data("000001.SH", start="20200101")
...
def super_signal(code):
    ts_code = f"{code}.SH" if len(code) == 6 else code
    df = get_data(ts_code, start="20250101")
